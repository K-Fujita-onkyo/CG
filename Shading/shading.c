...
static void PhongModel(GLfloat* pe, GLfloat* normal, GLfloat* out_color) {
  out_color[0] = 0.f;
  out_color[1] = 0.f;
  out_color[2] = 0.f;

  int i,j;
  GLfloat pe_n[3],pe_n_length=0.f,naiseki=0.f,R[3],cosfai=0.f,faim,V[3],v_length,r_length;
  
  for(i=0;i<g_num_lights;i++){

    //Ambient light
    for(j=0;j<3;j++){
      out_color[j]+=g_mat_amb[j]*g_light_amb[i][j];
    }
    ...
    for(j=0;j<3;j++){
      pe_n[j]=g_light_pos[i][j]-pe[j];
    }
    pe_n_length=sqrt((pe_n[0]*pe_n[0])+(pe_n[1]*pe_n[1])+(pe_n[2]*pe_n[2]));

    for(j=0;j<3;j++){
      naiseki+=normal[j]*(pe_n[j]/pe_n_length);
    }
    
    for(j=0;j<3;j++){
      out_color[j]+=g_mat_diff[j]*g_light_diff[i][j]*fmaxf(naiseki,0.0f);
    }

    //Phang-specular-reflection
    ...
    for(j=0;j<3;j++){
      R[j]=(pe_n[j]/pe_n_length)-2*(normal[j]*naiseki);  
    }

    r_length=sqrt(R[0]*R[0]+R[1]*R[1]+R[2]*R[2]);

    for(j=0;j<3;j++){
      cosfai+=(R[j]/r_length)*(V[j]/v_length);
    }
    cosfai=fmaxf(cosfai,0.0f);
    faim=cosfai;
    
    for(j=1;j<g_mat_shiny;j++){
      faim=faim*cosfai;
    }

    for(j=0;j<3;j++){
      out_color[j]+=g_mat_spec[j]*g_light_spec[i][j]*faim;
    }
    
    // printf("%f,%f,%f,%f\n",out_color[0],out_color[1],out_color[2],cosfai);
