...
void computeTriangleNormals(TriangleMesh* tri_mesh)
{
  ...
  for(i = 0; i < tri_mesh->_number_triangles; i++){
    ...
    sub(tri_mesh->_vertices[v1], tri_mesh->_vertices[v0], &p2_1);
    sub(tri_mesh->_vertices[v2], tri_mesh->_vertices[v0], &p3_1);
    computeCrossProduct(p2_1, p3_1, &tri_mesh->_triangle_normals[i]);
    normalize(tri_mesh->_triangle_normals[i], &tri_mesh->_triangle_normals[i]);
  }
}
...
void computeVertexNormals(TriangleMesh* tri_mesh)
{
  ...
  for(i=0;i<tri_mesh->_number_vertices;i++){
    ...
    tri_mesh->_vertex_normals[i]._x = 0;
    tri_mesh->_vertex_normals[i]._y = 0;
    tri_mesh->_vertex_normals[i]._z = 0;
...
void heatStep(TriangleMesh* tri_mesh) {
  ...
  int i,j;
  int sum_neighbors;//隣接してる頂点の個数
  int number_vj;//隣接してる頂点の番号
  Vector3 sigma_vj;//隣接してる頂点ベクトルの和
  Vector3 l_vi;//L(vi)
  float lam=0.1;//ラムダ
  ...
  for(i=0;i < tri_mesh->_number_vertices;++i){
    // 頂点iに隣接する頂点の数を取得。
    sum_neighbors = getNumberAdjacentVertices(tri_mesh, i);
    ...
    for (j = 0; j < sum_neighbors; ++j) {
      number_vj = getAdjacentVertex(tri_mesh,i,j);//隣接している頂点ベクトルの番号を手に入れる。
      add(sigma_vj,tri_mesh->_vertices[number_vj], &sigma_vj);//sigma(vj)を作る。
    }

    mulAV(1.0f/(float)sum_neighbors,sigma_vj,&sigma_vj);//(1/Ni)sigma(vj)を作る→ sigma_vj再利用

    sub(sigma_vj,tri_mesh->_vertices[i],&l_vi);//sigma(vj)-viをしてL(vi)を求める。
    mulAV(lam,l_vi,&l_vi);//ラムダかける。
    add(tri_mesh->_vertices[i],l_vi,&tri_mesh->_vertices[i]);//vi=vi+lam(L(vi)を実行している。
